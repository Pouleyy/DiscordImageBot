import mongoose from "../server/mongo.js";
...
CategorySchema.statics = {
  ...
  createOrUpdate(category) {
    return this.get(category.name).then(exist => {
      let cat;
      if (exist) {
        cat = exist;
      } else {
  ...
  /**
   * Remove one Category
   * @param {String} category Name of the category to delete
   * @returns {Promise}
   */
  remove(category) {
    return this.deleteOne({ name: category });
  },
